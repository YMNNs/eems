<template>
  <div>
    <!--    页头-->
    <a-page-header sub-title="管理接报，当发生紧急事件时工作人员创建接报，并提交领导审批，指挥人员对接报的审批无法决定，提交给专家处理，专家可以对接报进行处理。" title="接报维护"
                   @back="() => $router.go(-1)">
      <template #tags>
        <a-tag color="green">专家</a-tag>
      </template>
      <br>
      <a-row type="flex">
    ...
    >
      <a-descriptions :column="2" bordered size="small" title="接报信息">
        ...
        <a-descriptions-item v-if="caseInfo.specialistComment" :span="2" label="专家意见">{{
            caseInfo.specialistComment
          }}
        </a-descriptions-item>
      ...
      <a-form v-if="caseInfo.status==='等待审批'" ref="formRef" :model="form" :rules="rules" layout="vertical">
        <a-form-item label="专家意见" name="comment">
          <a-textarea
